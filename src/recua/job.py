...
class TransferJob:
    ...
    Attributes:
        source:         URL or URI of the remote resource.
                        Supported schemes (v1): https://, http://
                        Planned: s3://, file://
        dest:           Absolute or relative path for the local output file.
                        Parent directories will be created if absent.
        name:           Human-readable label used in progress display and logs.
                        Defaults to the basename of dest if not provided.
        expected_size:  Content-Length in bytes if known ahead of time.
                        Allows the engine to skip already-complete files without
                        issuing a HEAD request.
        priority:       Lower value = higher priority (min-heap convention).
                        Default 0. Jobs with equal priority are FIFO-ordered.
        meta:           Arbitrary caller-supplied key/value data.
                        Passed through to callbacks unchanged.
                        Treat as read-only after submission — the dataclass is
                        frozen but dict contents are not enforced immutable.
